...
//On button click this calls quicksort function
...
var partition=function(arr,left,right){
	...
	while(left<=right){
		...
		if(left<=right){
			...
			setTimeout(swap(arr, left, right, pivot), 1000);
...
function swap(arr, left, right, pivot){
			...
			temp=right-1;
			$left.animate({height:'+=100px',opacity:'1',color: 'red',background: 'yellow'},5000);
			$left.before($right);
